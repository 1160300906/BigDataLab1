...
//		conf.setDouble("max",ratelist.get(ratelist.size()-1));
//		conf.setDouble("min",ratelist.get(0));
		FileSystem fs = FileSystem.get(conf);
		fs.delete(new Path(outputpath), true);
		Job job = Job.getInstance(conf, "Filtered");
	    job.setJarByClass(PreprocessedMain.class);
	    job.setMapperClass(PreProcessedMapper.class);
	    job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(Text.class);
	    job.setReducerClass(PreprocessedReducer.class);
